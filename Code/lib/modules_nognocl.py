...
# 无感知损失的
############   GAN   ############
def train(dataloader, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D, writer,
          args):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        writer.add_scalar('errD_real', errD_real, epoch)
        writer.add_scalar('errD_fake', errD_fake, epoch)
        writer.add_scalar('errD_mis', errD_mis, epoch)
        writer.add_scalar('errD_MAGP', errD_MAGP, epoch)
        writer.add_scalar('errD', errD, epoch)
        # update D
        if args.mixed_precision:
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            ...
            x_normalized = 2 * ((fake - fake.min()) / (fake.max() - fake.min())) - 1
            ...
            writer.add_scalar('errG', errG, epoch)
            writer.add_scalar('loss_1', loss_1, epoch)
            writer.add_scalar('g_errG', g_errG, epoch)
...
def test(dataloader, text_encoder, netG, img_save_dir, PTM, device, epoch, max_epoch, times, z_dim, batch_size):
    PSNR = calculate_PSNR(dataloader, text_encoder, netG, img_save_dir, PTM, device, epoch, max_epoch, times, z_dim,
                          batch_size)
    return PSNR
