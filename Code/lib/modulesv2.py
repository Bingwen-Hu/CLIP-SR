...
def train(dataloader, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D, writer,args):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            ...
            SR = netG(LR, sent_emb)
            pred_d_real=netD(HR)
            l_d_real=gan_criterion(pred_d_real,True)
            

            pred_d_fake=netD(SR.detach())
            l_d_fake=gan_criterion(pred_d_fake,False)
        # with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
        errD=l_d_real+l_d_fake
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            fake_feats=netD(SR)
            l_d_realss=gan_criterion(fake_feats,True,False)
            ...
            loss_1 = loss_func(SR, HR)
            errG=loss_1+l_d_realss  
