...
class CLIP_Adapter(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, G_ch, CLIP_ch, cond_dim, k, s, p, map_num, CLIP):
        ...
        self.CrossAttention=CrossAttention(dim=64)

    def forward(self,out,c,noise):
        ...
        for FBlock in self.FBlocks:
            out = FBlock(out,c)
            out=self.CrossAttention(noise,out)
...
class NetG(nn.Module):
    def __init__(self, ngf, nz, cond_dim, imsize, ch_size, mixed_precision, CLIP):
        ...
        self.CrossAttention=CrossAttention(64)
        #(k,s,p)
        # 上采样网络
        # self.up = nn.Upsample();
    ...
    def forward(self, noise, c, eval=False): # x=noise, c=ent_emb
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else dummy_context_mgr() as mp:
            ...
            #cond(5,612)
            out=self.head(noise)
            outs=self.CrossAttention(noise,c)
            print(out.shape)
            exit(0)
            ...
            out = self.mapping(self.fc_code(out).view(out.size(0), self.code_ch, self.code_sz, self.code_sz), cond,noise)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        k = self.key(y).transpose(1, 2) # [batch_size, dim, seq_len_y]
        v = self.value(y) # [batch_size, seq_len_y, dim]
        
        energy = torch.bmm(q, k) # [batch_size, seq_len_x, seq_len_y]
        attention = nn.functional.softmax(energy, dim=-1) # [batch_size, seq_len_x, seq_len_y]
        
        z = torch.bmm(attention, v) # [batch_size, seq_len_x, dim]
        return z
