...
class NetG(nn.Module):
    def __init__(self, ngf, nz, cond_dim, imsize, ch_size, mixed_precision, CLIP):
        ...
        self.TBlocks=  M_Block(64, 32, 64, cond_dim, 3, 1, 1)
        ...
        nn.Conv2d(in_channels=3,
                    out_channels=56, kernel_size=5, stride=1, padding=2),
        
        nn.PReLU(),
        nn.Conv2d(in_channels=56,
                    out_channels=56, kernel_size=5, stride=2, padding=2),
        nn.PReLU(),
        nn.Conv2d(in_channels=56,
        ...
        nn.PReLU(),
        nn.Conv2d(in_channels=64,
                    out_channels=64, kernel_size=2, stride=1, padding=0),
        # M_Block(64, 32, 64, cond_dim, 3, 1, 1)
    )
        #(k,s,p)
        # 上采样网络
        # self.up = nn.Upsample();
    ...
    def forward(self, LR, c, eval=False): # x=noise, c=ent_emb
        with torch.cuda.amp.autocast() if self.mixed_precision and not eval else dummy_context_mgr() as mp:
            ...
            c=c.float()
            # LR_fuse=self.TBlocks(LR_feature,c)
            # print("LR_fuse",LR_fuse.size())
      
        
            out = self.mapping(self.fc_code(out).view(out.size(0), self.code_ch, self.code_sz, self.code_sz), cond,LR_feature)
