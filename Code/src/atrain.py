...
def main(args):
    ...
    if args.state_epoch!=1:
        ...
        # path=r"G:\workspaces\GALIP-main\GALIP-main\code\saved_models\bird\sr\state_epoch_040.pth"
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        if epoch % gen_interval == 0:
            # --------fixed_z是从真实图片中随机采样的噪声，要换成LR
            sample(LR, fixed_sent, netG, args.multi_gpus, epoch, args.img_save_dir, writer)
            print("finfish")
            torch.cuda.empty_cache()
                    '''
        # training
        ...
        if flag == 0:
            #torch.cuda.empty_cache()
            #------------计算PSNR的
            PSNR = test(valid_dl, text_encoder, netG, CLIP4evl, args.device, m1, s1, epoch, args.max_epoch, args.sample_times, args.z_dim, args.batch_size)
            torch.cuda.empty_cache()
            print(PSNR)
        ...
        if (args.multi_gpus==True) and (get_rank() != 0):
            ...
            # if epoch%test_interval==0:
            if epoch == 0:
                writer.add_scalar('FID', PSNR, epoch)
                # writer.add_scalar('CLIP_Score', TI_score, epoch)
                print('The %d epoch FID: %.2f, CLIP_Score: %.2f' % (epoch,PSNR))
            end_t = time.time()
