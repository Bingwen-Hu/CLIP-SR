...
from lib.modules_nognocl import sample_one_batch as sample, test as test, train as train
...
def main(args):
    ...
    # 2023_03_13_19_55_47
    stamp = '_'.join(
        [str(args.model), 'nf' + str(args.nf), str(args.stamp), str(args.CONFIG_NAME), str(args.imsize), time_stamp])
    ...
    print('**************else: ',
          params_count(CLIP4trn) + params_count(CLIP4evl) + params_count(image_encoder) + params_count(text_encoder))
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        train(train_dl, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D,
              writer, args)
        torch.cuda.empty_cache()
        # ==============================================save============================================================
        ...
        if epoch % test_interval == 0:
            # ------------计算PSNR的
            PSNR = test(valid_dl, text_encoder, netG, None, CLIP4evl, args.device, epoch, args.max_epoch,
